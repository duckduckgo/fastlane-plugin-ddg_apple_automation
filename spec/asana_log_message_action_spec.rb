describe Fastlane::Actions::AsanaLogMessageAction do
  describe "#run" do
    ...
    before do
      ...
      allow(Fastlane::Helper::DdgAppleAutomationHelper).to receive(:get_asana_user_id_for_github_handle).and_return(assignee_id)
    ...
    it "takes assignee id from github handle when is manual release" do
      expect(Fastlane::Helper::DdgAppleAutomationHelper).to receive(:get_asana_user_id_for_github_handle).with(github_handle)
